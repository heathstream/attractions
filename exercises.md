...
- In readme-clr1.txt, database-rebuild-all.ps, database-rebuild-all.sh, appsettings.json, secrets.json file rename `sql-Attractions`to name `sql-Attractions`
...
```csharp
    public DbSet<CreditCardDbM> Attractions { get; set; }
```
...
```csharp
// ...existing code...
        var Attractions = seeder.ItemsToList<CreditCardDbM>(nrItems);
...
- Rename the new folder to Attractions
- In the Attractions folder rename files GoodFriends.slnx and GoodFriends.code-workspace 
  to Attractions.slnx and Attractions.code-workspace
- Delete the .git folder in Attractions

Open Attractions in Visual Studio Code by double clicking on Attractions.code-workspace
- Rename GoodFriends.slnx to Attractions.slnx in tasks.json (3 places)
